-- migrations/048_livrer_commande_echeance.sql
-- Ajoute le paramètre p_date_reglement à livrer_commande, pour saisir
-- l'échéance de règlement au moment de la livraison (§18.3, §18.9).
-- COALESCE(p_date_reglement, date_reglement_prevue) : un appel sans échéance
-- (comptant, ou à 2 arguments) conserve la valeur existante au lieu de l'écraser.
-- Fichier recréé a posteriori en v26.37 depuis pg_get_functiondef (joué en base
-- en v26.36 ; l'ancienne surcharge (uuid, jsonb) a été supprimée par DROP, §18.3).

CREATE OR REPLACE FUNCTION public.livrer_commande(
    p_commande_id uuid,
    p_lignes jsonb,
    p_date_reglement date DEFAULT NULL::date
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    -- 4. Cloturer la commande + poser l'echeance de reglement
    -- COALESCE : si p_date_reglement est NULL (paiement comptant,
    -- ou appel a 2 arguments), on conserve la valeur existante
    -- au lieu de l'ecraser.
    UPDATE commandes
       SET statut = 'LIVREE',
           date_reglement_prevue = COALESCE(p_date_reglement, date_reglement_prevue)
    ...
    RETURN jsonb_build_object(
        ...
        'total', v_total,
        'ecritures', v_nb_ecr,
        'date_reglement', p_date_reglement
    );
END;
$function$;