-- ============================================================
-- Migration 014 — Fix B6 : CRU exclut uniquement Achat stock
-- AviGest v26.4
-- Problème migration 010 :
--   filtre IN ('Alimentation','Vaccin','Médicament','Poussin')
--   → exclut à tort Salaire, Prestation, Transport, Glace, Autre
-- Règle correcte (Bible v26) :
--   Tout entre dans le CRU SAUF 'Achat stock'
--   (mouvement de trésorerie, pas une charge consommée)
-- Correction : remplacer IN par NOT IN ('Achat stock')
-- ============================================================

DROP VIEW IF EXISTS vue_dashboard_bande;
...
SELECT
    ...
    COALESCE((SELECT sum(j.montant) AS sum FROM journal j
        WHERE j.bande_id = b.id
        AND j.type_ecriture = 'DEPENSE'::text), 0::numeric) AS total_depenses,
    -- FIX B6 : CRU = toutes dépenses SAUF 'Achat stock'
    -- Ancienne règle (migration 010) : IN ('Alimentation','Vaccin','Médicament','Poussin')
    -- Nouvelle règle : NOT IN ('Achat stock')
    COALESCE((SELECT sum(j_cru.montant) AS sum FROM journal j_cru
        WHERE j_cru.bande_id = b.id
        AND j_cru.type_ecriture = 'DEPENSE'::text
        AND j_cru.categorie != 'Achat stock'
        ), 0::numeric) AS total_depenses_cru,
    COALESCE((SELECT sum(j2.montant) AS sum FROM journal j2
