...
> **Synchronisation** : cette version `.md` est générée à partir de `bible_avigest_v26.docx` --- dernière synchronisation le **22/07/2026 (session v26.30).**. Le `.docx` reste la référence unique pour toute modification manuelle ; ce fichier `.md` est une copie dérivée destinée à être lue par Claude Code depuis le repo GitHub, à côté de `SCHEMA.md`. Ne jamais éditer ce `.md` comme source --- toujours régénérer depuis le `.docx` à jour.
...
| ### Table               | ### Rôle                                                                                                                                    |
+=========================+=============================================================================================================================================+
| ### bandes              | ### Lots de poulets --- id, ferme_id, nom, race, effectif_initial, date_arrivee, statut, is_deleted                                         |
+-------------------------+---------------------------------------------------------------------------------------------------------------------------------------------+
...
24.9 Chantier futur --- Rentabilité par bande

Distinct de la trésorerie (§24.2). Objectif : voir pour chaque bande son résultat économique (coûts vs recettes, marge, CRU). S\'appuiera sur journal.bande_id (déjà présent) et get_dashboard (qui calcule déjà une partie : marge nette, CRU par bande --- §14.2, §15.1). À diagnostiquer avant de coder : ce que get_dashboard renvoie déjà, pour ne pas reconstruire l\'existant. Non urgent.

💡 En clair : la caisse (fait), la rentabilité par bande (à faire plus tard). Le bande_id du journal attend déjà ce futur écran.

┌─────────────────────────────────────────────────────────────────┐ │ BLOC 8 --- Après édition du .docx │ └─────────────────────────────────────────────────────────────────┘

Une fois le .docx à jour, demander à Claude de régénérer le .md (pandoc), jamais l\'inverse (règle de synchronisation §en-tête).

--- Fin de la mise à jour Bible session v26.39 ---

*--- Fin de la Bible AviGest v26 --- Version .md générée le 30/07/2026 (session v26.39), à répercuter manuellement dans le .docx ---*
